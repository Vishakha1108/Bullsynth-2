    ...
    <div className="dash-card p-5 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-bg-elevated border border-border-subtle rounded-lg p-3">
          ...
          <div className="text-base font-bold font-mono text-accent">{portfolio ? fmtUsd(portfolio.cash_balance) : '—'}</div>
        </div>
        <div className="bg-bg-elevated border border-border-subtle rounded-lg p-3">
            ...
            <tbody>
              {trades.map((t) => (
                <tr key={t.id} className="dash-table-row">
                  ...
                  <td className="px-5 py-3 font-mono text-[11px] text-text-secondary">{t.session_id ?? '—'}</td>
      ...
      <div className="max-w-300 mx-auto px-6 py-8 flex flex-col gap-6">
        ...
        <div className="dash-card p-5 flex flex-col gap-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <select
              ...
              className="bg-bg-terminal border border-border-subtle rounded px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-accent/40"
            >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[10px] font-semibold uppercase tracking-wider text-text-secondary dash-table-header sticky top-0">
                    ...
                    <th className="text-right px-5 py-3">Realized PnL</th>
                  </tr>
                </thead>
                <tbody>
                  {sessions.map((s) => (
        ...
        <div className="dash-card p-4">
          <button
            ...
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-bg-elevated border border-border-subtle text-text-secondary hover:text-text-primary transition-colors text-xs font-semibold cursor-pointer"
          >
            <ArrowDownRight size={12} />
            Reset Scope To Overall
          </button>
          <div className="text-xs text-text-secondary mt-2">
            This page is fully backed by admin API data for bot, portfolio, trades, KPI, and session history.
          </div>
        </div>
      </div>
    </div>
