...
export function TickerSearch() {
  ...
  useEffect(() => {
    ...
    const handleOpenEvent = (e: Event) => {
      setMode((e as CustomEvent).detail?.mode || "search");
      setIsOpen(true);
    };
  ...
  const tickers = useMemo(() => {
    ...
    const inferCategory = (symbol: string, rawCategory?: string) => {
      ...
      if (SYNTHETIC_SYMBOLS.has(sym) || cat.includes("synthetic"))
        return "Synthetic";
      return "Stocks";
    };
    ...
    if (!normalized.some((t) => t.category === "Synthetic")) {
      ...
      const symbolsToPromote = preferredSynthetic
        .filter((s) => fallbackSymbols.includes(s))
        .slice(0, 2);
      const finalPromotions =
        symbolsToPromote.length > 0
          ? symbolsToPromote
          : fallbackSymbols.slice(0, 2);
  ...
  useEffect(() => {
    if (!isOpen) return;
    setActiveIndex(0); // eslint-disable-line react-hooks/set-state-in-effect -- reset index on filter change
  }, [query, tab, mode, isOpen]);

  useEffect(() => {
    if (!isOpen) return;
    setActiveIndex((current) => { // eslint-disable-line react-hooks/set-state-in-effect -- clamp index to list bounds
            ...
            <div className="tv-modal-header">
              <div className="tv-modal-title-row">
                <span className="tv-modal-title">
                  {mode === "compare" ? "Compare Symbol" : "Symbol Search"}
                </span>
                <button
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((t, index) => {
                  ...
                  return (
                    <div
                      key={t.symbol}
                      className="tv-modal-item-wrapper flex items-center group"
                    >
                      <button
                      ...
                      >
                        <div className="tv-modal-item-left">
                          <div className="tv-modal-item-symbol">
                            ...
                            compareSymbols.includes(t.symbol) ? (
                              <span className="tv-modal-item-check text-red-500 text-xs ml-2">
                                Remove
                              </span>
                        ...
                        <div className="tv-modal-item-right">
                          <span className="tv-modal-item-category">
                            {t.category}
                          </span>
                        </div>
                      </button>
...
function OHLCVOverlay() {
  ...
  return (
    <div className="ohlcv-overlay">
      ...
      <span className="ohlcv-value" style={{ color }}>
        {o.toFixed(2)}
      </span>
      ...
      {compareSymbols.map((sym, idx) => {
        ...
        return (
          <div
            ...
            style={{ color: compareColors[idx % compareColors.length] }}
          >
            <span>{sym}</span>
...
function IndicatorsOverlay() {
  ...
  const enabledCustomScripts = customIndicatorScripts.filter(
    (script) => script.enabled,
  );
  if (!enabledIndicators.length && !enabledCustomScripts.length) return null;

  const labels = INDICATOR_LIBRARY.reduce<Record<string, string>>(
    (acc, item) => {
  ...
  return (
    <div className="chart-indicators-overlay">
      {enabledIndicators.map((id) => (
        <span
          key={id}
          className="chart-indicator-tag"
          style={{ borderColor: INDICATOR_COLORS[id] }}
        >
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="text-red-500 p-10 font-mono text-xs whitespace-pre-wrap flex flex-col h-full bg-slate-900 border overflow-auto">
...
function Chart() {
  ...
  const [activeTrigger, setActiveTrigger] = useState<{ id: string; symbol: string; targetPrice: number; type: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const handleTrigger = (e: Event) => {
      setActiveTrigger((e as CustomEvent).detail);
    };
  ...
  const currentCrosshairRef = useRef<{ time: number; price: number } | null>(
    null,
  );

  const indicatorSeriesRef = useRef<IndicatorSeriesBucket>({
    line: [],
    histogram: [],
  });
  ...
  const removeAllIndicatorSeries = useCallback(() => {
    ...
    indicatorSeriesRef.current.line.forEach((series) =>
      chartRef.current?.removeSeries(series),
    );
    indicatorSeriesRef.current.histogram.forEach((series) =>
      chartRef.current?.removeSeries(series),
    );
  ...
  const addHistogramIndicator = useCallback(
  ...
  useEffect(() => {
    ...
    const showAxisCrosshair = ["crosshair", "dot", "arrow"].includes(
      activeTool,
    );
    ...
    if (existingCandles.length > 0 && Array.isArray(existingCandles)) {
      mainSeries.setData(
        existingCandles.map(formatCandleData) as Parameters<
          typeof mainSeries.setData
        >[0],
      );
      volumeSeries.setData(
        existingCandles.map((c) => ({
          time: c.time as UTCTimestamp,
          value: c.volume,
          color:
            c.close >= c.open
  ...
  useEffect(() => {
    ...
    if (compareSymbols.length > 0) {
      chart
        .priceScale("right")
        .applyOptions({ mode: PriceScaleMode.Percentage });
    } else {
    ...
    compareSymbols.forEach((sym, idx) => {
      ...
      if (compareCandlesData && compareCandlesData.length > 0) {
        const lineData = compareCandlesData.map((c) => ({
          time: c.time as UTCTimestamp,
          value: c.close,
        }));
        ...
        for (const point of lineData) {
          if (
            deduped.length === 0 ||
            deduped[deduped.length - 1].time !== point.time
          ) {
            deduped.push(point);
          } else {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const cycleWatchlistSymbol = (direction: 1 | -1) => {
        ...
        const symbols = (
          state.watchlist.length > 0 ? state.watchlist : state.symbols
        ).filter(Boolean);
        ...
        const nextIndex =
          (baseIndex + direction + symbols.length) % symbols.length;
        ...
        if (currentCrosshairRef.current) {
          ...
          setDrawings([
            ...currentDrawings,
            {
              ...
              points: [
                {
                  time: currentCrosshairRef.current.time,
                  price: currentCrosshairRef.current.price,
                },
              ],
            },
          ]);
      ...
      // Ctrl + H: Hide/Show drawings
        ...
        setDrawingsHidden((prev) => {
          ...
          window.dispatchEvent(
            new CustomEvent("show-toast", {
              detail: next ? "Drawings hidden" : "Drawings shown",
            }),
          );
          return next;
        });
  ...
  }, [latestCandle, formatCandleData, currentSymbol, candles]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      (enabledIndicators.length === 0 &&
        !customIndicatorScripts.some((script) => script.enabled))
    )
      return;
          ...
          .map((value, idx) =>
            value == null
              ...
              : {
                  ...
                  value,
                  color:
                    value >= 0 ? "rgba(38,166,154,0.5)" : "rgba(239,83,80,0.5)",
                },
          )
          .filter(
            (
              point,
            ): point is { time: UTCTimestamp; value: number; color: string } =>
              point !== null,
          ),
      );
    ...
    if (enabledIndicators.includes("ichimoku")) {
      ...
      const tenkanS = chart.addSeries(LineSeries, {
        color: "#2962FF",
        lineWidth: 1,
        priceLineVisible: false,
        lastValueVisible: false,
      });
      const kijunS = chart.addSeries(LineSeries, {
        color: "#E91E63",
        lineWidth: 1,
        priceLineVisible: false,
        lastValueVisible: false,
      });
      const senkouAS = chart.addSeries(LineSeries, {
        color: "#4CAF50",
        lineWidth: 1,
        priceLineVisible: false,
        lastValueVisible: false,
      });
      const senkouBS = chart.addSeries(LineSeries, {
        color: "#FF5722",
        lineWidth: 1,
        priceLineVisible: false,
        lastValueVisible: false,
      });
      const chikouS = chart.addSeries(LineSeries, {
        color: "#9C27B0",
        lineWidth: 1,
        lineStyle: 2,
        priceLineVisible: false,
    ...
    if (enabledIndicators.includes("adx")) {
      ...
      const plusS = chart.addSeries(LineSeries, {
        color: "#4ade80",
        lineWidth: 1,
        priceScaleId: scaleId,
        priceLineVisible: false,
      ...
      const minusS = chart.addSeries(LineSeries, {
        color: "#ef4444",
        lineWidth: 1,
        priceScaleId: scaleId,
        priceLineVisible: false,
    ...
    if (enabledIndicators.includes("aroon")) {
      ...
      const downS = chart.addSeries(LineSeries, {
        color: "#ef4444",
        lineWidth: 1,
        priceScaleId: scaleId,
        priceLineVisible: false,
    ...
    if (enabledIndicators.includes("stoch")) {
      ...
      const scaleId = "stoch";
      const kS = chart.addSeries(LineSeries, {
          ...
          .map((value, idx) =>
            value == null
              ...
              : {
                  ...
                  value,
                  color:
                    value >= 0 ? "rgba(38,166,154,0.7)" : "rgba(239,83,80,0.7)",
                },
          )
          .filter(
            (p): p is { time: UTCTimestamp; value: number; color: string } =>
      ...
      <div className="chart-body">
        ...
        <div className="chart-canvas-wrap">
          ...
          <div
            ...
            className={`chart-canvas ${screenshotFlash ? "animate-flash" : ""} ${
              activeTool === "arrow"
                ? "cursor-arrow"
                : activeTool === "dot"
                  ? "cursor-dot"
            ...
            <div className="absolute inset-0 bg-black/60 z-[300] flex items-center justify-center">
              <div className="bg-bg-elevated p-8 rounded-xl border-2 border-bull text-center shadow-2xl shadow-bull/20 text-text-primary min-w-[350px]">
                ...
                <p className="text-xl mb-6">
                  ...
                  has{" "}
                  {activeTrigger.type === "crossing"
                    ? "crossed"
                    : activeTrigger.type}{" "}
                  <span className="font-bold text-white">
                    ${activeTrigger.targetPrice.toFixed(2)}
                  </span>
                  !
                </p>
                <button
                  className="tv-alert-cta text-lg w-full"
